...
BASE_CMD="export OPENAI_API_KEY=$OPENAI_API_KEY && python3 -m tests.decision_tester.decision_tester \
  --model unsloth/Phi-3-mini-4k-instruct \
  --dataset all"

# Test 1
LOG1="$LOG_DIR/gpu_test_rag_$(date +'%Y%m%d_%H%M%S').txt"
echo "Running Test 1 (with --rag)... Output: $LOG1"
eval "$BASE_CMD --rag" | tee "$LOG1"

# Test 2
LOG2="$LOG_DIR/gpu_test_rag_offline_$(date +'%Y%m%d_%H%M%S').txt"
echo "Running Test 2 (with --rag --rag_offline)... Output: $LOG2"
eval "$BASE_CMD --rag --rag_offline" | tee "$LOG2"

# Test 3
LOG3="$LOG_DIR/gpu_test_no_rag_$(date +'%Y%m%d_%H%M%S').txt"
